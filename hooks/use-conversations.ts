...
async function fetchConversations(
  ...
  if (collectionId !== undefined) {
    params.append("collectionId", collectionId === null ? "null" : collectionId);
  }
...
export function useConversations(options: UseConversationsOptions = {}) {
  const { page = 1, limit = 20, collectionId, enabled = true } = options;
  const queryClient = useQueryClient();

  // Query for fetching conversations
  const conversationsQuery = useQuery({
    queryKey: ["conversations", { page, limit, collectionId }],
    queryFn: () => fetchConversations(page, limit, collectionId),
    enabled,
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, ...data }: { id: string; title?: string; collectionId?: string | null }) =>
    ...
    generateTitle: async (conversationId: string) => {
      try {
        ...
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('titleGenerating', { 
        ...
        if (response.ok) {
          ...
          if (typeof window !== 'undefined') {
            window.dispatchEvent(new CustomEvent('titleGenerated', { 
              detail: { conversationId } 
            }));
          }
