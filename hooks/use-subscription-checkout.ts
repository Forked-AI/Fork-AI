...
export type SubscriptionBillingScope =
	| { customerType: "user"; referenceId?: undefined }
	| { customerType: "organization"; referenceId: string };
...
export function useSubscriptionCheckout(scope?: SubscriptionBillingScope) {
		...
		async (annual: boolean) => {
			...
			try {
				...
				const upgradeMethod =
					...
					subscriptionClient?.upgradeSubscription;

				if (typeof upgradeMethod !== "function") {
					throw new Error(
						"Subscription checkout is unavailable right now. Please try again later."
					);
				}
				...
				if (!redirectUrl) {
					throw new Error(
						"Checkout URL was not returned. Please try again in a moment."
					);
				}
