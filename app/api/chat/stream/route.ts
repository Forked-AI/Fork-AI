...
export async function POST(request: Request) {
	try {
		...
		// 4. Check rate limit (skip for guests)
		let rateLimit: any = null;
		if (!isGuest) {
			rateLimit = await checkChatRateLimit(userId!);
	...
	const readableStream = new ReadableStream({
		async start(controller) {
			try {
				...
				// Send conversation ID first (for new conversations, authenticated users only)
				...
				// Send user message ID (authenticated users only)
				...
				// Save assistant message to database (authenticated users only)
				if (!isGuest && conversation) {
					...
					// For guests, just send done without saving
					controller.enqueue(
						encoder.encode(
							`data: ${JSON.stringify({
								type: "done",
...
"X-RateLimit-Remaining": String(isGuest ? 0 : rateLimit?.remaining || 0),
"X-RateLimit-Reset": isGuest ? new Date().toISOString() : (rateLimit?.resetAt?.toISOString() || new Date().toISOString()),
		},
	});
} catch (error) {
