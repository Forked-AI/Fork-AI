...
const createShareSchema = z.object({
	...
	messageSelections: z
		.array(shareMessageSelectionSchema)
		.min(1)
	...
	try {
		...
		return await withJsonIdempotency(
			...
			async () => {
				...
				const selectedMessageIds = messageSelections.map(
				...
				const orderedMessages = sortMessagesBySelection(
					messages,
					messageSelections
				).map((message) => ({
					...message,
				...
				const selectedContent = orderedMessages
					.map((message) => message.content)
					.join("\n\n");
				const moderationDecision = evaluateAggregateContentModeration({
					contents: orderedMessages.map((message) => message.content),
				...
				return {
					...
					resourceType: "shared_conversation",
					resourceId: share.id,
				};
			}
		);
	...
	try {
		...
		return NextResponse.json({
			shares: shares.map((share) => ({
				...
				messageCount: (JSON.parse(share.selectedMessageIds) as string[])
					.length,
