...
const previewShareSchema = z.object({
	...
	selectedMessageIds: z.array(z.string().min(1)).min(1).max(MAX_PREVIEW_MESSAGES),
	autoMaskPII: z.boolean().optional().default(true),
	generateSummary: z.boolean().optional().default(false),
	approvedFindingIdsByMessageId: z.record(z.array(z.string().min(1)).max(30)).optional().default({}),
	redactedMessageIds: z.array(z.string().min(1)).max(MAX_PREVIEW_MESSAGES).optional().default([]),
})

function sortMessageIds<T extends { id: string }>(messages: T[], messageIds: string[]) {
...
export async function POST(request: Request) {
	try {
		...
		const orderedMessages = sortMessageIds(messages, selectedMessageIds).map((message) => ({
			...message,
